...
#Windows
command = 'node C:\path\to\kimi-hud\statusline.mjs'
# 可选：脚本失败时回退到这个内置槽位布局
...
- 套餐用量仅适用于 kimi 托管账号（需要 `credentials/kimi-code.json` 中的 OAuth token）；access token 过期且 kimi 尚未刷新时，会短暂显示上一次缓存的数据。正常情况下数据最长滞后约 120 秒（缓存周期）；接口持续故障时重试退避到每 120 秒一次，期间显示最后一次成功的数据，超过 24 小时未更新则该段隐藏。
